    ...
    class Pravilo {
        ...
        function stampaj_tabelu_pravila($name_kriterijum=NULL) {
            try {
                ...
                echo "<tr><th colspan='4'><h2 style='margin: 0'>Terms of use</h2></th></tr>";   
        ...
        function dodaj($name, $file) { 
            try {
                $sql="INSERT INTO terms_of_use (name, file) VALUES ('$name', '$file')";
        ...
        function brisi($id) {   
            try {
                $sql="DELETE FROM terms_of_use WHERE id=$id";
                $pdo_izraz=$this->dbh->exec($sql);
                return true;
            }
            catch(PDOException $e) {
        ...
        function izmeni($id, $name, $file) {   
            try {
                $sql="UPDATE terms_of_use SET name=:name, file=:file WHERE id=:id";   
        ...
        function uzmi_podatke_o_pravilu($id) {
            try {
                $sql="SELECT * FROM terms_of_use WHERE id=$id";
                $pdo_izraz=$this->dbh->query($sql);
