    ...
    class Slika {
        ...
        function stampaj_tabelu_slike($name_kriterijum=NULL) {
            try {
                ...
                echo "<tr><th colspan='4'><h2 style='margin: 0'>Images 2019</h2></th></tr>";   
        ...
        function dodaj($name, $path) { 
            try {
                $sql="INSERT INTO images2019 (name, path) VALUES ('$name', '$path')";
        ...
        function brisi($id) {   
            try {
                $sql="DELETE FROM images2019 WHERE id=$id";
                $pdo_izraz=$this->dbh->exec($sql);
                return true;
            }
            catch(PDOException $e) {
        ...
        function izmeni($id, $name, $path) {   
            try {
                $sql="UPDATE images2019 SET name=:name, path=:path WHERE id=:id";   
        ...
        function uzmi_podatke_o_slici($id) {
            try {
                $sql="SELECT * FROM images2019 WHERE id=$id";
                $pdo_izraz=$this->dbh->query($sql);
